  ...
  describe('Test Box', () => {
    it('should sample correctly with scalar low and high', () => {
      ...
      for (let i = 0; i < sample.data.length; i++) {
        expect(sample.data[i]).to.be.lt(1);
        expect(sample.data[i]).to.be.gt(0);
      }
    });
    it('should sample correctly with ndarray low and high', () => {
      const space = new Spaces.Box(np.pack([[0, 1], [2, 3]]), np.pack([[1, 2], [3, 4]]), [2, 2], "float32");
      let sample = space.sample();
      for (let i = 0; i < sample.data.length; i++) {
        expect(sample.data[i]).to.be.lt(i + 1);
        expect(sample.data[i]).to.be.gt(i);
      }
    });
    it('should sample correctly with ndarray low and scalar high', () => {
      const space = new Spaces.Box(np.pack([[0, 1], [2, 3]]), 10, [2, 2], "float32");
      let sample = space.sample();
      for (let i = 0; i < sample.data.length; i++) {
        expect(sample.data[i]).to.be.lt(10);
        expect(sample.data[i]).to.be.gt(i);
      }
    });
    it('should sample correctly with scalar low and ndarray high', () => {
      const space = new Spaces.Box(-10, np.pack([[0, 1], [2, 3]]), [2, 2], "float32");
      let sample = space.sample();
      for (let i = 0; i < sample.data.length; i++) {
        expect(sample.data[i]).to.be.lt(i + 1);
        expect(sample.data[i]).to.be.gt(-10);
      }
    });
